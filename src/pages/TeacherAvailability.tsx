...
const TEACHERS = [
  ...
  { id: '5', name: 'محمد عبدالباري', subject: 'حساب، تفسير', color: 'bg-teal-500' },
...
function TeacherDetailModal({ teacher, sessions, onClose, language }: TeacherDetailModalProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 p-4" onClick={onClose}>
      ...
      >
        <div className={`${teacher.color} px-6 py-5 flex items-center justify-between`}>
          <button onClick={onClose} className="p-2 bg-white bg-opacity-20 hover:bg-opacity-30 rounded-xl transition-colors">
            <X className="w-5 h-5 text-white" />
          </button>
          <div className="flex items-center gap-4">
            <div className="text-right">
              <h2 className="text-xl font-bold text-white">{teacher.name}</h2>
              <p className="text-white text-opacity-80 text-sm">{teacher.subject}</p>
            </div>
            <div className="w-14 h-14 bg-white bg-opacity-20 rounded-2xl flex items-center justify-center">
              <span className="text-white text-2xl font-bold">{teacher.name.charAt(0)}</span>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-4 px-6 py-4 bg-gray-50 border-b border-gray-200">
          <div className="flex items-center gap-3 bg-white rounded-xl px-4 py-3 border border-gray-200">
            ...
            <div>
              ...
              <p className="text-xl font-bold text-gray-900">{weekSessions.length}</p>
            </div>
          </div>
          <div className="flex items-center gap-3 bg-white rounded-xl px-4 py-3 border border-gray-200">
            ...
            <div>
              <p className="text-xs text-gray-500">{language === 'ar' ? 'إجمالي الحصص' : 'Total Sessions'}</p>
              <p className="text-xl font-bold text-gray-900">{teacherSessions.length}</p>
            </div>
          </div>
          <div className="flex items-center gap-3 bg-white rounded-xl px-4 py-3 border border-gray-200">
            ...
            <div>
              <p className="text-xs text-gray-500">{language === 'ar' ? 'الطلاب' : 'Students'}</p>
              <p className="text-xl font-bold text-gray-900">{uniqueStudents.length}</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-auto p-6" dir="rtl">
          {tab === 'week' ? (
            <>
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2 text-xs">
                  <span className="flex items-center gap-1"><span className="w-3 h-3 rounded bg-red-200 border border-red-300 inline-block" />{language === 'ar' ? 'مشغول' : 'Busy'}</span>
                  <span className="flex items-center gap-1"><span className="w-3 h-3 rounded bg-green-100 border border-green-200 inline-block" />{language === 'ar' ? 'متاح' : 'Free'}</span>
                </div>
              ...
              {weekSessions.length > 0 && (
                <div className="mb-4 bg-primary-light border border-primary rounded-xl p-4">
                  <p className="text-sm font-semibold text-primary mb-2">{language === 'ar' ? 'حصص هذا الأسبوع' : 'This Week Sessions'}</p>
                  <div className="space-y-2">
                    {weekSessions.map(s => (
                      <div key={s.id} className="flex items-center justify-between bg-white rounded-lg px-3 py-2 border border-primary">
                        <span className="text-xs text-gray-500">{s.time} - {s.endTime}</span>
                        <div className="flex items-center gap-3">
                          <span className="text-xs badge-primary px-2 py-0.5 rounded-full">{s.subject}</span>
              ...
              <div className="overflow-x-auto rounded-xl border border-gray-200">
                <table className="w-full text-xs">
                  <thead>
                    <tr className="bg-gray-50 border-b border-gray-200">
                      ...
                      {weekDates.map((date, idx) => {
                        ...
                        return (
                          <th key={idx} className={`px-2 py-2.5 text-center font-medium min-w-[90px] ${isToday ? 'bg-primary-light text-primary' : 'text-gray-600'}`}>
                            <div>{DAYS[date.getDay()]}</div>
                            <div className={`font-normal text-xs ${isToday ? 'text-primary' : 'text-gray-400'}`}>{date.getDate()}/{date.getMonth() + 1}</div>
                          </th>
                        );
                      })}
                    </tr>
                  ...
                  <tbody>
                    {HOURS.map(hour => (
                      <tr key={hour.value} className="border-b border-gray-100 hover:bg-gray-50">
                        ...
                        {weekDates.map((date, idx) => {
                          ...
                          const info = busy ? getSlotInfo(date, hour.value) : null;
                          const isToday = formatDate(date) === today;
                          return (
                            <td key={idx} className={`px-1 py-1 text-center ${isToday ? 'bg-primary-light' : ''}`}>
                              {busy && info ? (
                                <div className="bg-red-100 border border-red-200 rounded-md px-1 py-1 group relative cursor-default">
                                  <div className="flex items-center justify-center gap-1">
                                    <XCircle className="w-3 h-3 text-red-500 flex-shrink-0" />
                                    <span className="text-red-700 text-xs font-medium truncate max-w-[60px]">{info.studentName.split(' ')[0]}</span>
                                  </div>
                                  <div className="absolute z-10 bottom-full right-0 mb-1 bg-gray-900 text-white text-xs rounded-lg p-2.5 w-44 hidden group-hover:block shadow-lg">
                                    <p className="font-medium mb-1">{info.studentName}</p>
                                    <p className="text-gray-300 flex items-center gap-1"><Clock className="w-3 h-3" />{info.time} - {info.endTime}</p>
                                    <p className="text-gray-300 flex items-center gap-1"><BookOpen className="w-3 h-3" />{info.subject}</p>
                                  </div>
                                </div>
                              ) : (
            ...
            <>
              <div className="flex items-center justify-between mb-4">
                <div className="text-sm font-semibold text-gray-700">{monthName}</div>
                <div className="flex items-center gap-2 bg-white border border-gray-200 rounded-xl px-4 py-2">
                  <button onClick={() => setMonthOffset(m => m - 1)} className="p-1 hover:bg-gray-100 rounded-lg">
                    <ChevronRight className="w-4 h-4 text-gray-600" />
                  </button>
                  <span className="text-sm font-medium text-gray-800 px-2">{monthName}</span>
                  <button onClick={() => setMonthOffset(m => m + 1)} className="p-1 hover:bg-gray-100 rounded-lg">
                    <ChevronLeft className="w-4 h-4 text-gray-600" />
                  </button>
                </div>
              ...
              <div className="grid grid-cols-7 gap-1">
                ...
                {monthDates.map(date => {
                  ...
                  return (
                    ...
                    >
                      <span className={`text-xs font-medium block text-center mb-1 ${isToday ? 'text-primary' : hasSessions ? 'text-red-700' : 'text-gray-500'}`}>
                        {date.getDate()}
                      </span>
                      ...
                      {daySessions.length > 2 && (
                        <span className="text-xs text-red-600 font-medium">+{daySessions.length - 2}</span>
                      )}
                    </div>
                  );
                })}
              ...
              {uniqueStudents.length > 0 && (
                <div className="mt-6">
                  <h3 className="text-sm font-semibold text-gray-800 mb-3">{language === 'ar' ? 'الطلاب المسجلون مع هذا المعلم' : 'Enrolled Students'}</h3>
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                    {uniqueStudents.map(studentName => {
                      const studentSessions = teacherSessions.filter(s => s.studentName === studentName);
                      return (
                        <div key={studentName} className="flex items-center gap-2 bg-white border border-gray-200 rounded-xl px-3 py-2.5">
                          <div className="w-8 h-8 bg-primary-light rounded-full flex items-center justify-center flex-shrink-0">
                            <span className="text-primary text-xs font-bold">{studentName.charAt(0)}</span>
                          </div>
                          <div className="min-w-0">
                            <p className="text-sm font-medium text-gray-800 truncate">{studentName}</p>
                            <p className="text-xs text-gray-400">{studentSessions.length} {language === 'ar' ? 'حصة' : 'sessions'}</p>
                          </div>
                        </div>
                      );
                    })}
...
export default function TeacherAvailability() {
  ...
  const busySlots = useMemo(() => {
    const slots: Record<string, { teacherName: string; studentName: string; time: string; endTime: string; subject: string }[]> = {};
  ...
  const isTeacherBusy = (teacherName: string, date: Date, hour: number): boolean => {
    const dateStr = formatDate(date);
  ...
  const displayedTeachers = selectedTeacherId === 'all'
    ? TEACHERS
  ...
  return (
    <div className="p-6 space-y-6" dir="rtl">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">{language === 'ar' ? 'المعلمون المتاحون' : 'Teacher Availability'}</h1>
          <p className="text-gray-500 text-sm mt-1">{language === 'ar' ? 'جدول توافر المعلمين الأسبوعي' : 'Weekly teacher availability schedule'}</p>
        </div>
        <div className="flex items-center gap-3">
          <button
      ...
      {viewMode === 'teacher' ? (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
          {teacherStats.map(teacher => (
            <div key={teacher.id} className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden hover:shadow-md transition-shadow">
              <div className={`${teacher.color} px-5 py-4`}>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-white text-opacity-80 text-xs">{teacher.subject}</p>
                  </div>
                </div>
              </div>
              <div className="p-5 space-y-3">
                <div className="grid grid-cols-2 gap-3">
                  <div className="bg-gray-50 rounded-xl p-3 text-center">
                    <p className="text-xs text-gray-500 mb-1">{language === 'ar' ? 'هذا الأسبوع' : 'This Week'}</p>
                    <p className="text-2xl font-bold text-gray-900">{teacher.weekSessions}</p>
                    <p className="text-xs text-gray-400">{language === 'ar' ? 'حصة' : 'sessions'}</p>
                  </div>
                  <div className="bg-gray-50 rounded-xl p-3 text-center">
                ...
                <div className="flex flex-wrap gap-1">
                  {DAYS_AR.map((day, idx) => {
                    ...
                    const hasSessions = sessions.some(
                      s => s.teacherName === teacher.name && s.date === dateStr
                    );
                    return (
                      <div
                        key={day}
                        className={`flex-1 min-w-0 py-1.5 rounded-lg text-center text-xs font-medium ${
                          hasSessions ? `${teacher.color} text-white` : 'bg-gray-100 text-gray-400'
                        }`}
                        title={day}
                      >
                        {day.slice(0, 1)}
                ...
                <div className="flex items-center justify-between pt-1">
                  ...
                  <div className="flex items-center gap-2">
                    <div className={`w-2 h-2 rounded-full ${teacher.weekSessions > 0 ? 'bg-red-400' : 'bg-green-400'}`} />
                    <span className={`text-xs font-medium ${teacher.weekSessions > 0 ? 'text-red-600' : 'text-green-600'}`}>
                      {teacher.weekSessions > 0
                        ? `${language === 'ar' ? 'مشغول' : 'Busy'} (${teacher.weekSessions})`
                        : (language === 'ar' ? 'متاح هذا الأسبوع' : 'Available this week')
                      }
                    </span>
                  </div>
                </div>
        ...
        <>
          ...
          <div className="flex gap-4 text-xs">
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-gray-600">{language === 'ar' ? 'مشغول - لديه حصة' : 'Busy - Has Session'}</span>
            </div>
            <div className="flex items-center gap-1.5">
              <div className="w-3 h-3 rounded bg-green-100 border border-green-200" />
              <span className="text-gray-600">{language === 'ar' ? 'متاح' : 'Available'}</span>
            </div>
          </div>

          {displayedTeachers.map(teacher => (
            <div key={teacher.id} className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
              <div className={`${teacher.color} px-5 py-3 flex items-center justify-between`}>
                <button
                  onClick={() => setSelectedTeacherForModal(teacher)}
                ...
                <div className="flex items-center gap-3">
                  <div>
                    <p className="text-white font-bold text-sm text-right">{teacher.name}</p>
                    <p className="text-white text-xs opacity-80 text-right">{teacher.subject}</p>
                  </div>
                ...
                <table className="w-full text-xs">
                  <thead>
                    <tr className="bg-gray-50 border-b border-gray-200">
                      ...
                      {weekDates.map((date, idx) => (
                        <th key={idx} className="px-2 py-2 text-center text-gray-600 font-medium min-w-[90px]">
                          <div>{DAYS[date.getDay()]}</div>
                          <div className="text-gray-400 font-normal">{date.getDate()}/{date.getMonth() + 1}</div>
                        </th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {HOURS.map(hour => (
                      <tr key={hour.value} className="border-b border-gray-100">
                        ...
                        {weekDates.map((date, idx) => {
                          const busy = isTeacherBusy(teacher.name, date, hour.value);
                          const info = busy ? getSlotInfo(teacher.name, date, hour.value) : null;
                          return (
                              ...
                              {busy && info ? (
                                <div className="bg-red-100 border border-red-200 rounded-md px-1 py-1 group relative cursor-default">
                                  ...
                                  <div className="absolute z-10 bottom-full right-0 mb-1 bg-gray-900 text-white text-xs rounded-lg p-2 w-40 hidden group-hover:block shadow-lg">
                                    <p className="font-medium">{info.studentName}</p>
                                    <p className="text-gray-300">{info.time} - {info.endTime}</p>
                                    <p className="text-gray-300">{info.subject}</p>
                                  </div>
                                </div>
                              ) : (
                                <div className="bg-green-50 border border-green-100 rounded-md py-1 flex items-center justify-center">
