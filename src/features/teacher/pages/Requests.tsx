...
export default function TeacherRequests() {
  ...
  const [requests, setRequests] = useState<TeacherRequest[]>([
    {
      id: 'REQ-T-001',
      type: isRtl ? 'طلب إجازة سنوية' : 'Annual Leave',
      date: '2023-11-20',
      status: 'pending',
      reason: isRtl ? 'أرجو الموافقة على طلب إجازة لمدة أسبوع بداية من الشهر القادم.' : 'I request a one-week leave starting next month.',
    },
    {
      id: 'REQ-T-002',
      ...
      date: '2023-11-15',
      status: 'approved',
      reason: isRtl ? 'تأجيل حصة الرياضيات ليوم غد لوجود ظرف صحي.' : 'Postponing math session to tomorrow due to health reasons.',
      reply: isRtl ? 'تم التأجيل وإبلاغ الطلاب.' : 'Postponed and students notified.'
    },
    {
      id: 'REQ-T-003',
      type: isRtl ? 'سلفة من الراتب' : 'Salary Advance',
      date: '2023-10-10',
      status: 'rejected',
      reason: isRtl ? 'طلب سلفة بقيمة 500 دولار.' : 'Requesting a $500 advance.',
      reply: isRtl ? 'لا يمكن إتمام الطلب هذا الشهر.' : 'Cannot fulfill this request this month.'
    }
  ]);

  const [isModalOpen, setIsModalOpen] = useState(false);
  const [newRequestType, setNewRequestType] = useState(isRtl ? 'طلب إجازة' : 'Leave Request');
  ...
  const getStatusBadge = (status: TeacherRequest['status']) => {
    switch (status) {
      case 'approved':
        return (
              ...
              <div className="space-y-5">
                <div>
                  ...
                  >
                    ...
                    <option value={isRtl ? 'سلفة مالية' : 'Salary Advance'}>{isRtl ? 'سلفة مالية' : 'Salary Advance'}</option>
